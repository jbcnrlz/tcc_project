...
?>
<?php Pjax::begin(['id' => 'gridApresentacao','options'=>[
                                    'enablePushState' => false,
                                    'timeout'=>false,
                                ]]);
                         echo GridView::widget([
                            'dataProvider' => $modelApresentacao->search(Yii::$app->request->queryParams),
                            'columns' => [
                                [
...
[
            ...
            'value' => function($data) {
                if ($data->status == 0) {
                    return "<span class='glyphicon glyphicon-remove text-danger size-icone' data-toggle='tooltip' title='Apresentação Cancelada'></span>";
                } else if ($data->status == 1) {
                    return "<span class='glyphicon glyphicon-time text-success size-icone' data-toggle='tooltip' title='Aguardando a Apresentação'></span>";
                } else {
                    return "<span class='glyphicon glyphicon-ok text-info size-icone' data-toggle='tooltip' title='Apresentação Realizada'></span>";
                }
            }
        ],
                                ...
                                [
                                    ...
                                    'template'=>'{view}{atualizar}{print}{del}',
                                     'dropdown' => false,
                                     'vAlign' => 'middle',
                                      'width' => '140px',
                        ...
                        return Html::a('<span class="glyphicon glyphicon-eye-open"></span>', ['visualizar?id='.$model->id], [
                           ...
                           'export'=>(Mimin::checkRoute($this->context->id.'/exportar'))?'':false,
                            ...
                            'panel' => [
                                ...
                                'type'=>'info',
                                'before'=>"<span class='pull-left' style='font-size:20px; color:#C21D16; font-weight:400'>:: Lista de Projetos Agendados</span>".$this->render('_filtrar',['model'=>$modelApresentacao,'tab'=>'agendado']),                                                                                                                                                          'after'=>Html::a('<i class="glyphicon glyphicon-repeat"></i> Reset List', ['index'], ['class' => 'btn btn-info']),
                                'showFooter'=>false
                            ],
                        'exportConfig'=> [
