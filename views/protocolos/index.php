...
<div class="protocolos-index">
    ...
    <div class="box">      
        <div class="box-body">
           ...
           $protocolos = new app\models\Protocolos();
           $curso = \app\models\Curso::find()->all();
           $listaClassificacoes = $protocolos->classificacoes;
           $optionClassificacao = '<option value=""></option>';
           foreach ($listaClassificacoes as $key => $valor){
               $selected = (@$_POST['filtro']['classificacao']==$key)?'selected':'';
               $optionClassificacao .=  '<option value="'.$key.'" '.$selected.'>'.$valor.'</option>';
           }
...
$botaoFiltro = '
                <a class="btn btn-warning isDisabled" href="/protocolos/cadastrar" data-pjax="0"><i class="glyphicon glyphicon-plus"></i> Adicionar</a>
                 <div class="clear-fix"></div>
                 <div class="filtro" style="margin-right:10px; margin-top:10px;">
                 
                <form name="filtroGrid" action="/protocolos/index" class="form-stylo" method="post">
                <input type="hidden" name="_csrf" value="<?=Yii::$app->request->getCsrfToken()?>" />
            <div class="col-md-2" style="padding-left:0px">
            <label>Classificação</label>
        <select id="filtro-etapa" class="form-control" name="filtro[classificacao]" aria-required="true" aria-invalid="true">
                   '.$optionClassificacao.'
                    </select></div><div class="col-md-3" style="padding-left:0px">
                    <label>Curso</label>
        ...
        <label>Período</label>
        '.DatePicker::widget([
    'name' => 'filtro[dta_inicial]',
    'value' => @$_POST['filtro']['dta_inicial'],
    'type' => DatePicker::TYPE_RANGE,
    'name2' => 'filtro[dta_final]',
    'value2' => @$_POST['filtro']['dta_final'],
    'separator'=>'Até',
    'pluginOptions' => [
        'autoclose'=>true,
        'format' => 'dd/mm/yyyy'
        ...
        <input class="form-control somente-numero-5" type="text" value="'.@$_POST['filtro']['numero'].'" name="filtro[numero]">
        </div>
     ...
     echo GridView::widget([
        ...
        'columns' => [
         ...
         [
                    'attribute' => 'descricao',
                    'width'=>'250px'],
                    ['attribute'=>'dta_protocolo', 'width'=>'','format'=>['datetime','php:d/m/Y']],

            [
                'class' => 'kartik\grid\ActionColumn',
                'template'=>'{print}{update}{del}',
                 'dropdown' => false,
                 'vAlign' => 'middle',
                  'width' => '100px',
    ...
    return Html::a('<span class="glyphicon glyphicon-print"></span>', ['imprimir-protocolo?id='.$model->id], [
                                                                      'title' => 'Imprimir',
                                                                      'data-toggle' => 'tooltip',
